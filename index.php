...
// 1. นับจำนวนสินค้าในตะกร้า (คงเดิม)
$cart_count = 0;
...
<head>
    ...
    <style>
        ...
        .filter-btn { display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; border-radius: 10px; color: #555; text-decoration: none; font-size: 0.85rem; transition: 0.3s; }
        ...
        #search_results { position: absolute; width: 100%; background: white; z-index: 9990; margin-top: 10px; border-radius: 20px; box-shadow: 0 20px 50px rgba(0,0,0,0.15); display: none; overflow: hidden; border: 1px solid #eee; }
        ...
        <div class="col-lg-3 d-none d-lg-block">
            <div class="sticky-top" style="top: 110px;">
                <div class="cat-group">
                    ...
                    <div class="cat-header">ประเภท (Categories)</div>
                    <a href="index.php?cat=รองเท้า" class="filter-btn <?php echo ($cat == 'รองเท้า') ? 'active' : ''; ?>">รองเท้า</a>
                    <a href="index.php?cat=เสื้อผ้า" class="filter-btn <?php echo ($cat == 'เสื้อผ้า') ? 'active' : ''; ?>">เสื้อผ้า</a>
                    <a href="index.php?cat=อุปกรณ์กีฬา" class="filter-btn <?php echo ($cat == 'อุปกรณ์กีฬา') ? 'active' : ''; ?>">อุปกรณ์เสริม</a>

                    <div class="cat-header">เลือกตามเพศ (Gender)</div>
                    <a href="index.php?cat=ชาย" class="filter-btn <?php echo ($cat == 'ชาย') ? 'active' : ''; ?>">ผู้ชาย</a>
                    <a href="index.php?cat=หญิง" class="filter-btn <?php echo ($cat == 'หญิง') ? 'active' : ''; ?>">ผู้หญิง</a>
                </div>
            </div>
        </div>

        <div class="col-lg-9">
            <h2 class="fw-bold mb-4"><?php echo ($cat != '') ? 'หมวดหมู่: ' . $cat : 'สินค้าแนะนำ'; ?></h2>
            <div class="row g-4">
                <?php
                // --- Smart SQL Logic: ดึงข้อมูลจากทั้ง p_category และ p_gender ---
                $sql = "SELECT * FROM products WHERE 1";
                if ($search) { $sql .= " AND (p_name LIKE '%$search%' OR p_brand LIKE '%$search%')"; }
                if ($cat) {
                    if ($cat == 'ชาย' || $cat == 'หญิง') {
                        $sql .= " AND p_gender = '$cat'";
                    } else {
...
$(document).ready(function(){
    ...
    $('.add-to-cart-btn').click(function() {
        if (!<?php echo $is_logged_in ? 'true' : 'false'; ?>) {
            Swal.fire({ title: 'กรุณาเข้าสู่ระบบ', icon: 'warning', confirmButtonColor: '#111' });
            return;
        }
