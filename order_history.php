...
// 2. ดึงข้อมูลออเดอร์เฉพาะของผู้ใช้ที่ล็อกอิน
...
<div class="container pb-5" style="max-width: 900px;">
    <?php if (mysqli_num_rows($result) > 0): ?>
        <?php while($row = mysqli_fetch_array($result)): 
            // ดึงภาพสินค้าชิ้นแรกมาแสดง
            $items = explode(',', $row['o_product']);
            $first_item = trim($items[0]);
            $clean_name = preg_replace('/\s\(x\d+\)$/', '', $first_item);
                ...
                <div class="row align-items-center">
                    <div class="col-auto">
                        <div class="product-thumbnail-wrapper">
                            <img src="<?php echo $p_image; ?>" class="product-thumbnail">
                        </div>
                    </div>
                    ...
                    <div class="col ms-md-2">
                        ...
                        <div class="fw-bold text-dark text-truncate mb-1" style="max-width: 350px;">
                    ...
                    <div class="col-md-3 text-md-end mt-3 mt-md-0 ps-md-4 border-start">
                        ...
                        <button class="btn btn-dark btn-sm rounded-pill px-4 w-100 fw-bold py-2" 
                                onclick="showOrderData('<?php echo str_pad($row['o_id'], 5, '0', STR_PAD_LEFT); ?>', '<?php echo $status; ?>', '<?php echo htmlspecialchars($row['o_name']); ?>', '<?php echo $row['o_phone']; ?>', '<?php echo addslashes($row['o_address']); ?>', '<?php echo addslashes($row['o_product']); ?>', '<?php echo number_format($row['o_total']); ?>', '<?php echo $st_class; ?>')">
                            รายละเอียด
                        </button>
                    </div>
                </div>
                ...
                <div class="row g-2">
                    <div class="col-6">
                        <div class="detail-box">
                            ...
                            <div class="small fw-bold" id="v-name"></div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="detail-box">
                            <div class="detail-label">เบอร์โทรศัพท์</div>
                            <div class="small fw-bold" id="v-phone"></div>
                        </div>
                    </div>
                </div>
                <div class="detail-box">
...
<script>
function showOrderData(id, status, name, phone, address, items, total, stClass) {
    document.getElementById('v-id').innerText = 'ORDER #' + id;
    document.getElementById('v-status').innerText = status;
    document.getElementById('v-status').className = 'status-pill ' + stClass;
    ...
    document.getElementById('v-items').innerText = items;
    document.getElementById('v-total').innerText = '฿' + total;
    
    // เรียก Modal ของ Bootstrap ให้แสดงผล
